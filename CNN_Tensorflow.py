...
convolutional_layers = [(32, 5), (64, 5), (128, 5)]
learningRate =  1e-4
iterations = 100000
drop_out_rate = 0.2
cifar10_mean = None
...
def build_model(input_layer, hidden_nodes, conv_nodes, num_classes, drop_rate, phase):
	prev_layer = tf.reshape(input_layer, [-1, *img_shape_full]) #32, 32, 3])

	for num_filters, size_kernel in conv_nodes:
		prev_layer = tf.layers.conv2d(inputs=prev_layer, filters=num_filters, kernel_size=[size_kernel, size_kernel], padding="same", activation=tf.nn.relu)
